...
const seedExpenses: Expense[] = [
  { id: 1, amount: 22000, note: "April Rent", paymentMethod: "CARD", categoryId: 2, date: "2026-04-01" },
  { id: 2, amount: 1240, note: "Weekly groceries", paymentMethod: "CASH", categoryId: 1, date: "2026-04-04" },
  { id: 3, amount: 499, note: "Netflix subscription", paymentMethod: "CARD", categoryId: 5, date: "2026-04-06" },
  { id: 4, amount: 800, note: "Uber rides this week", paymentMethod: "CARD", categoryId: 4, date: "2026-04-05" },
  { id: 5, amount: 3200, note: "New shoes", paymentMethod: "CARD", categoryId: 6, date: "2026-04-03" },
];
...
export default function ExpensesPage() {
  ...
  const filtered = React.useMemo(() => {
    return expenses.filter((e) => {
      const cat = getCategoryLabel(e.categoryId);
      const matchSearch =
        e.note.toLowerCase().includes(searchQuery.toLowerCase()) ||
        cat.toLowerCase().includes(searchQuery.toLowerCase());
      const matchCat =
        filterCategory === "all" || e.categoryId === Number(filterCategory);
  ...
  return (
    <div className="min-h-screen flex bg-zinc-50 dark:bg-zinc-950">
        ...
        <header className="sticky top-0 z-30 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border-b border-zinc-100 dark:border-zinc-800 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 font-medium mt-0.5">Manage your spending</p>
            </div>
          </div>
          <Button
            variant="gradient"
          ...
          <Card className="border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm overflow-hidden">
            {filtered.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="font-semibold text-zinc-500 dark:text-zinc-400">No expenses found</p>
                <p className="text-sm text-zinc-400 dark:text-zinc-500 mt-1">Try adjusting your filters or add a new expense</p>
              </div>
            ) : (
              <div>
                ...
                <div className="divide-y divide-zinc-50 dark:divide-zinc-800">
                  {filtered.map((expense) => (
                    ...
                    >
                      <div>
                        <p className="font-semibold text-zinc-900 dark:text-zinc-50 text-sm">{expense.note}</p>
                        <span className="inline-flex items-center mt-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-rose-100 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400">
                          {getCategoryLabel(expense.categoryId)}
                        </span>
                      </div>
                      <span className="text-xs text-zinc-400 font-medium text-right">{expense.date}</span>
                      <span className={`text-xs font-bold text-right ${expense.paymentMethod === "CARD" ? "text-blue-500" : "text-emerald-500"}`}>
                        {expense.paymentMethod}
                      </span>
                      <span className="text-base font-extrabold text-rose-500 dark:text-rose-400 text-right">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800 px-4 py-3 flex justify-around z-40">
        {navItems.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => navigate(`/${key}`)} className={`flex flex-col items-center space-y-1 text-[10px] font-bold ${key === "expenses" ? "text-cyan-500" : "text-zinc-400"}`}>
            <Icon className="w-5 h-5" />
            <span>{label}</span>
          </button>
        ))}
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (errors.note) setErrors((p) => ({ ...p, note: "" }));
                    }}
                  />
